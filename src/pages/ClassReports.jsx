...
function ClassReports() {
            ...
            classes.forEach(c => {
                api.get(`/sessions/class/${c.id}`)
                    .then(res => {
                        ...
                        rawSessions.forEach(s => { if (s && s.sessionNum) uniqueMap.set(s.sessionNum, s); });
                        const sessions = Array.from(uniqueMap.values());
                        const completedCount = sessions.filter(s => s && (s.status === 'completed' || s.status === 'cancelled')).length;
    ...
    useEffect(() => {
        if (selectedReport) {
            const studentsInReport = allStudents.filter(s => s && s.classCode === selectedReport.classCode);
                        ...
                        if (session && (session.status === 'completed' || session.status === 'cancelled')) {
                            api.get(`/attendance/${selectedReport.id}/${session.sessionNum}`)
                                .then(attRes => {
                                    ...
                                    setAttendanceMap(prev => ({ ...prev, [session.sessionNum]: presentIds.size }));
                                })
                                .catch(() => { });
                        }
                    });
    ...
    const currentProgress = reportSessions.filter(s => s && (s.status === 'completed' || s.status === 'cancelled')).length;

    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', animation: 'fadeIn 0.3s ease-out' }}>

            {Object.keys(groupedClasses).length === 0 && <p style={{ color: 'var(--text-muted)' }}>Chưa có dữ liệu lớp học trên hệ thống.</p>}

            {Object.keys(groupedClasses).map(monthLabel => (
                <div key={monthLabel} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <h3 style={{ fontSize: '1.15rem', fontWeight: '800', color: 'var(--text-main)', borderBottom: '2px solid var(--primary-light)', paddingBottom: '8px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
                        {groupedClasses[monthLabel].map(c => {
                            ...
                            const totalSessSafe = Math.max(1, parseInt(c.totalSessions) || 19);
                    ...
                    <div style={{ width: '900px', maxHeight: '90vh', backgroundColor: '#f8fafc', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: 'var(--shadow-lg)' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', backgroundColor: 'white', borderBottom: '1px solid var(--border-color)' }}>
                            ...
                            <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border-color)' }}>
                                ...
                                <strong style={{ fontSize: '1rem', color: '#9333ea' }}>{currentProgress}/{Math.max(1, parseInt(selectedReport.totalSessions) || 19)} buổi</strong>
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', padding: '24px', flex: 1, minHeight: 0 }}>
                            ...
                            <div style={{ backgroundColor: 'white', borderRadius: '12px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                                ...
                                <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', flex: 1 }}>
                                    {reportSessions.filter(s => s && (s.status === 'completed' || s.status === 'cancelled')).length === 0 &&
                                    ...
                                    {reportSessions
                                        .filter(s => s && (s.status === 'completed' || s.status === 'cancelled'))
                                        ...
                                        .map((session) => (
                                            <div key={session.sessionNum} style={{ border: `1px solid ${session.status === 'completed' ? '#10b981' : '#ef4444'}`, borderRadius: '8px', padding: '12px', backgroundColor: 'var(--bg-card)' }}>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                                    ...
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                        <span style={{ fontSize: '0.8rem', fontWeight: '700', color: 'var(--success)' }}>
                                                            ...
                                                            {Math.min(attendanceMap[session.sessionNum] || 0, reportStudents.length)}/{reportStudents.length} HV đi học
                                                        </span>
                                                ...
                                                <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '8px' }}>{session.title || 'Chưa có tiêu đề'} - {session.notes || 'Không có ghi chú'}</p>
