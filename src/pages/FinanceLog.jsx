...
function FinanceLog() {
  ...
  const handleAddInvoice = async (e) => {
    ...
    if (
      ...
      addNotification(
        "Vui lòng điền tên học viên và ít nhất 1 loại tiền (VNĐ, JPY hoặc CNY)!",
        "error",
        "finance",
      );
      return;
    ...
    try {
      ...
      addNotification(
        "Hệ thống: Ghi nhận hóa đơn thu học phí thành công!",
        "success",
        "finance",
      );
    } catch (error) {
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      setInvoices(
        invoices.map((inv) => (inv.id === editingInvoice.id ? res.data : inv)),
      );
  ...
  const handleDelete = async (id) => {
    if (
      window.confirm(
        "Cảnh báo: Việc xóa hóa đơn sẽ làm thay đổi báo cáo doanh thu. Xác nhận xóa?",
      )
    ) {
      try {
  ...
  const totalCollectedVnd = invoices.reduce(
    (sum, inv) => sum + (inv.amount || 0),
    0,
  );
  const totalCollectedJpy = invoices.reduce(
    (sum, inv) => sum + (inv.amountJpy || 0),
    0,
  );
  const totalCollectedCny = invoices.reduce(
    (sum, inv) => sum + (inv.amountCny || 0),
    0,
  );
  return (
    <div className="FinanceLog-style-1">
      <div className="card FinanceLog-style-2">
        <div className="FinanceLog-style-3">
          <div>
            <span className="FinanceLog-style-4">
              Tổng dòng tiền học phí đã thu
            </span>
            <div className="FinanceLog-style-5">
              <h2 className="FinanceLog-style-6">
                {totalCollectedVnd.toLocaleString("vi-VN")} VNĐ
              </h2>
              {totalCollectedJpy > 0 && (
                <h2 className="FinanceLog-style-7">
                  + {totalCollectedJpy.toLocaleString("vi-VN")} ¥ (JPY)
                </h2>
              )}
              {totalCollectedCny > 0 && (
                <h2 className="FinanceLog-style-8">
      ...
      <div className="my-portal-grid FinanceLog-style-10">
        <div className="portal-left-column">
          <div className="card FinanceLog-style-11">
            ...
            <form onSubmit={handleAddInvoice} className="FinanceLog-style-13">
              ...
              <div>
                <label className="FinanceLog-style-15">
                  Tên học viên đóng phí
                </label>
                <input
                  type="text"
                  className="form-control"
              ...
              <div>
                <label className="FinanceLog-style-16">
              ...
              <div>
                <label className="FinanceLog-style-27">
              ...
              <button
                type="submit"
                className="btn btn-primary FinanceLog-style-30"
              >
        ...
        <div className="portal-right-column">
          <div className="card FinanceLog-style-31">
            ...
            <div className="modal-table-container FinanceLog-style-33">
              <table className="modal-table FinanceLog-style-34">
                <thead>
                  ...
                  {invoices.map((inv) => (
                    <tr key={inv.id} className="FinanceLog-style-42">
                      ...
                      <td className="FinanceLog-style-45">
                        <strong className="FinanceLog-style-46">
                          {inv.studentName}
                        </strong>
                        ...
                        {inv.notes && (
                          <span className="FinanceLog-style-48">
                            Lưu ý: {inv.notes}
                          </span>
                        )}
                      </td>
                      <td className="FinanceLog-style-49">
                        {(inv.amount > 0 ||
                          (!inv.amount &&
                            !inv.amountJpy &&
                            !inv.amountCny)) && (
                          <div className="FinanceLog-style-50">
                            {(inv.amount || 0).toLocaleString("vi-VN")} đ
                          </div>
                        )}
                        {inv.amountJpy > 0 && (
                          <div className="FinanceLog-style-51">
                            {inv.amountJpy.toLocaleString("vi-VN")} ¥ (JPY)
                          </div>
                        )}
                        {inv.amountCny > 0 && (
                          <div className="FinanceLog-style-52">
                            {inv.amountCny.toLocaleString("vi-VN")} ¥ (CNY)
                          </div>
                        )}
                      </td>
                      <td className="FinanceLog-style-53">
                        <span className="badge-studying FinanceLog-style-54">
                          {inv.status}
                        </span>
                      </td>
      ...
      {editingInvoice && (
        <div className="FinanceLog-style-59">
          <div className="card FinanceLog-style-60">
            <div className="FinanceLog-style-61">
              <h3 className="FinanceLog-style-62">
                <i className="fa-solid fa-file-invoice-dollar"></i> Sửa Hóa Đơn
                INV-{editingInvoice.id}
              </h3>
              <button
                onClick={() => setEditingInvoice(null)}
                className="FinanceLog-style-63"
              >
                ✖
              </button>
            </div>
            <div className="FinanceLog-style-64">
              <div>
                <label className="FinanceLog-style-65">
                  Ngày nộp (DD/MM/YYYY)
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="FinanceLog-style-67">
                  Khóa học / Nội dung
                </label>
                <input
                  type="text"
            ...
            <div className="FinanceLog-style-78">
              <button
                className="btn FinanceLog-style-79"
